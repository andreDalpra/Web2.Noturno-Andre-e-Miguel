...
function App() {
  const [user, setUser] = useState(() => localStorage.getItem('musicReviewsToken') ? JSON.parse(localStorage.getItem('musicReviewsUser') || 'null') : null)
  ...
  const nav = <header><button className="brand" onClick={() => setPage('feed')}>nota<span>musical</span></button><Search onSelect={chooseMusic}/><nav><button className="button ghost" onClick={() => setPage('playlists')}>Playlists</button><button className="button ghost" onClick={() => setPage('create')}>+ Review</button><button className="profile" onClick={() => setPage('profile')}>{user.nickname.slice(0, 1).toUpperCase()}<span>Minha conta</span></button><button className="logout" onClick={logout}>Sair</button></nav></header>
  return <><div className="app">{nav}{notice && <div className="notice">{notice}<button onClick={() => setNotice('')}>×</button></div>}{error && <div className="error">{error}</div>}
    ...
    {page === 'playlists' && <Playlists user={user} refresh={playlistRefresh} onError={setError} onChanged={() => setPlaylistRefresh(value => value + 1)} />}
  </div></>
}
...
function Playlists({ refresh, onError }) {
  const [playlists, setPlaylists] = useState([]); const [selected, setSelected] = useState(null)
  const [name, setName] = useState(''); const [collaborative, setCollaborative] = useState(false); const [nickname, setNickname] = useState(''); const [track, setTrack] = useState(null)
  const load = async () => { try { setPlaylists(await request('/playlists')) } catch (e) { onError(e.message) } }
  useEffect(() => { load() }, [refresh])
  const create = async e => { e.preventDefault(); try { const playlist = await request('/playlists', { method:'POST', body:JSON.stringify({name, collaborative}) }); setName(''); setSelected(playlist); await load() } catch (e) { onError(e.message) } }
  const open = async id => { try { setSelected(await request(`/playlists/${id}`)) } catch (e) { onError(e.message) } }
  const addParticipant = async e => { e.preventDefault(); try { setSelected(await request(`/playlists/${selected.id}/users/${encodeURIComponent(nickname)}`, {method:'POST'})); setNickname('') } catch (e) { onError(e.message) } }
  const addTrack = async music => { try { setSelected(await request(`/playlists/${selected.id}/tracks`, {method:'POST', body:JSON.stringify({spotify_id:music.id})})); setTrack(null) } catch (e) { onError(e.message) } }
  return <main><div className="page-heading"><p className="eyebrow">COLEÇÕES</p><h1>Minhas playlists</h1><p>Crie coleções e compartilhe músicas com outras pessoas.</p></div><div className="playlist-layout"><section><form className="playlist-create" onSubmit={create}><h2>Nova playlist</h2><input required value={name} onChange={e => setName(e.target.value)} placeholder="Nome da playlist"/><label className="check"><input type="checkbox" checked={collaborative} onChange={e => setCollaborative(e.target.checked)}/> Playlist colaborativa</label><button className="button primary">Criar playlist</button></form><div className="playlist-list">{playlists.map(item => <button className={selected?.id === item.id ? 'playlist-item selected' : 'playlist-item'} key={item.id} onClick={() => open(item.id)}><strong>{item.name}</strong><small>{item.participants.length} participante(s) · {item.collaborative ? 'Colaborativa' : 'Normal'}</small></button>)}{playlists.length === 0 && <Empty text="Você ainda não criou nenhuma playlist."/>}</div></section>{selected && <section className="playlist-detail"><p className="eyebrow">PLAYLIST</p><h2>{selected.name}</h2><p>{selected.collaborative ? 'Playlist colaborativa' : 'Playlist normal'}</p><h3>Participantes</h3><ul>{selected.participants.map(item => <li key={item.id}>{item.nickname}</li>)}</ul>{selected.collaborative && <form onSubmit={addParticipant} className="inline-form"><input required value={nickname} onChange={e => setNickname(e.target.value)} placeholder="Nickname do participante"/><button className="button primary">Adicionar</button></form>}<h3>Músicas</h3><div className="track-list">{selected.spotify_ids.map(id => <span key={id}>{id}</span>)}{selected.spotify_ids.length === 0 && <small>Nenhuma música adicionada.</small>}</div><Search onSelect={music => {setTrack(music); addTrack(music)}}/>{track && <p className="inline-success">{track.name} adicionada à playlist.</p>}</section>}</div></main>
}
